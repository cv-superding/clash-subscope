...
| 「导入到 Clash」点确认后 Clash 没关 | Clash Verge Rev 的 `clash-verge-service.exe` 会立刻拉起被关闭的主进程 | v1.2.7 起导入流程会**连服务一起杀**；杀 Windows 服务需管理员权限，**右键 clash-subscope.exe → 以管理员身份运行**即可 |
| 导入提示「无法自动关闭，请用任务管理器手动结束」 | 工具运行在非管理员模式，Windows 服务（`clash-verge-service.exe` / `FlClashHelperService.exe`）拒绝被终止 | **右键 clash-subscope.exe → 以管理员身份运行**；或先用任务管理器手动退出 Clash 主程序再导入 |
